...
export const chooseSpecialite = async (
  ...
  if (campagne.status !== StatusCampagne.ouverte) {
    throw new Error("Specialite choice is disabled. Wait for admin to open the campaign.");
  }

  const now = new Date();
  if (now < campagne.dateDebut || now > campagne.dateFin) {
    throw new Error("Specialite choice period is closed. It is only available during the campaign dates.");
  }
